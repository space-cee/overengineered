	...
	constructor(
		...
		const update = (face: Enum.NormalId, distance: number): void => {
			...
			const overscaled = blocks.any(
				(b) => b.block.PrimaryPart!.Size.div(b.origModel.PrimaryPart!.Size).findMax() > 999999999999999,
			);
			if (overscaled) {
				this.error.set("Some blocks are scaled too big (maximum is 256x)");
			} else {
				const underscaled = blocks.any(
					(b) => b.block.PrimaryPart!.Size.div(b.origModel.PrimaryPart!.Size).findMin() < 1 / 999999999999999,
				);
				if (underscaled) {
